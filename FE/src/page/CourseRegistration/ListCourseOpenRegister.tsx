        ...
        <table className="w-[96%] rounded-t-md overflow-hidden bg-[white] rounded-b-md">
          ...
          <div className="h-[400px] overflow-y-auto">
            {courses && courses.length && courses.map((course, index) => (
                <tr onClick={() => handleClick(course)} key={index} className="flex text-black text-center justify-between border-[#cccc]">
                  ...
                  <th className="border-[1px] flex-[2] text-[grey] font-normal">
                    {course.ngay}, {course.kip}, {course.tuan}
                  </th>
                  <th className="border-[1px] flex-[1]">
                    <Checkbox disabled={IsDisable(course)} checked={listCourseChecked.includes(course.mon_hoc_id) && !IsDisable(course)} />
                  </th>
                </tr>
            ))}
          </div>
